...
async def on_pm_s(client: Bot, message: Message):
    if check_ban := check_is_black_list(message):
        await message.reply_text(
            text=IS_BLACK_LIST_ED_MESSAGE_TEXT.format(reason=check_ban.reason)
        )
        return
    if SUB_CHANNEL != -100:
        try:
            ...
            return await message.reply_text(
                f"You Need to Join {chat.title}{username} in order to Use This Bot!."
            )
    fwded_mesg = None
    if message.edit_date:
        ...
        fwded_mesg = await message.forward(
            chat_id=AUTH_CHANNEL, disable_notification=True
        )
    ...
    add_user_to_db(
        fwded_mesg.message_id, message.from_user.id, message.message_id, 0, 0
    )
