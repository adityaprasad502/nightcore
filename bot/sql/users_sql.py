...
def add_user_to_db(message_id: int, chat_id: int, um_id: int, mu_id: int, kopp_id: int):
...
def get_user_id(message_id: int):
    ...
    try:
        if s__ := SESSION.query(Users).get(str(message_id)):
...
def get_chek_dmid(um_id: int):
    ...
    try:
        if all_lst := SESSION.query(Users).filter(Users.um_id == um_id).all():
            return all_lst[-1]
    finally:
        SESSION.close()
...
def get_chek_mdid(kopp_id: int):
    ...
    try:
        if all_lst := SESSION.query(Users).filter(Users.kopp_id == kopp_id).all():
            return all_lst[-1]
    finally:
        SESSION.close()
